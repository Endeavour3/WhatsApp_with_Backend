...
import { TextField, Button, CircularProgress, CardMedia, Grid, Box, Stack, SnackbarOrigin, Snackbar } from '@mui/material';
import { useRef, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAddContactMutation } from '../../store/contacts/contactApi';
...
export default function Registration() {
    ...
    const onSubmit: any = async (data: Data) => {
        try {
            ...
            const result: Result = await addContact(formData);
            if (result.error) {
                alert(result.error.data.message)
                // handleOpenOtpSnackbar({ vertical: 'top', horizontal: 'center' }, result.error.data.message)
            }
            if (result) {
                // console.log("first", result)
                alert(result.data.message)
                // alert(`${result.data.contactId}`)
                // handleOpenOtpSnackbar({ vertical: 'top', horizontal: 'center' }, result.data.message)
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        <Box component={"input"} {...register('profile_picture')} type="file" />
