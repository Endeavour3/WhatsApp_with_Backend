...
import { useGetMessagesByContactQuery } from "../../../store/messages/messageApi";
...
export default function DisplayMessages() {
    ...
    return (
        <>
            ...
            >
                <List>
                    {messages.filter((message) => message.send_to === Number(receiverId)).map((message, index, array) => (
                        <ListItem
                            key={message.id}
                            ref={index === array.length - 1 ? lastMessageRef : null}
                            sx={{
                                justifyContent: Number(id) === message.send_from ? "flex-end" : "flex-start",
                            }}
                        >
